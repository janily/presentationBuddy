...
function title(x, y, size, color, font, lines = ["AGENT", "SKILLS"], extra = "") {
  const weight = extra.includes("font-weight=") ? "" : 'font-weight="800"';
  const tracking = extra.includes("letter-spacing=") ? "" : 'letter-spacing="-2"';
  return `<text x="${x}" y="${y}" fill="${color}" font-family="${esc(font)},Noto Sans SC,sans-serif" font-size="${size}" ${weight} ${tracking} ${extra}>${lines.map((line, index) => `<tspan x="${x}" dy="${index ? size * .9 : 0}">${esc(line)}</tspan>`).join("")}</text>`;
}
